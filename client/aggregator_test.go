...
func TestClient_AggregatorPipelinesMetrics(t *testing.T) {
	...
	coreInstance, err := setupCoreInstance(nil, asUser.BaseURL, token)
	wantEqual(t, err, nil)
	defer func() {
		err = tearDownResource(nil, coreInstance)
		if err != nil {
			return
		}
	}()
	...
	t.Run("valid pipelines", func(t *testing.T) {
	...
	t.Run("zero pipeline ID", func(t *testing.T) {
		err = retry(func() error {
